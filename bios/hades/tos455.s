...
;letzte Aenderung vom :	14.04.1998

;********************************************************************************

;fa_27.3.95:patche TOS. "img" extension verwenden ist default im seka
;1. Drag TOS455.S (this file) on seka.tos to run it
;2a. assemble it by a<return> o<return>
;2b. load original TOS 3.06 image with ri<return> TOS306UK(for UK)<return> $300000<return> -1<return>
;2c. assemble again with a <return> o<return>
;3. Write new TOS image with wi<return> TOSH<return> $300000<return> $380000<return>
;3a. Quit SEKA by pressing Ctrl-C
;4. Drag HADESFIX.FIL on TOSPATCH.TTP to run tospatch
;5. TOS.IMG ist new TOS image file
...
;Language diff between $3400-$37C6 (max.)
;This must match the .NATION directive in hadesfix.fil
lang_dif:	equ -$5a
                   ;   0 = german
				   ;-$5A = usa/uk
				   ; $2C = swiss german
...
cpushala0:	equ $f4e8	;push und invalidate d+i cache line by a0
...
;neues init 
		...
		reset
        move.l  #$00ffc040,d0	;transparent 2: $8000'0000-ffff'ffff = no cache serialized
        dc.w 	movec,dtt1
        dc.w 	movec,itt1
...
not_zweiseitig:
		cmp.l   4.w,d3
		beq.s   zweiseitig
		moveq   #9,d6		; nur ein modul einseitig
		bra.s   mem_set_end
zweiseitig:
		cmp.w   #2,d6
		bgt.s   mem_set_end
...
;resetvektor testen und springen wenn ok
		lea	mtra1(pc),a6
		bra	romorg+$00c06         ;mem val test
mtra1:		bne	noreset		;wenn nicht ok kein reset vektor sprung
...
nomemchk:
		lea 	startup_stk,a7		;stack startup
		lea 	$a12.w,a0
		lea 	$e6fc,a1	
...
;***************************************************************************************
; Fix boot device error (BOOT_DEV from TOSPATCH)
		org  romorg+$00912
		load ramorg+$00912
		dc.w $3039

;***************************************************************************************
; Fix stack pointer bug (STACK_POINTER from TOSPATCH)
		org  romorg+$013D2
		load ramorg+$013D2
        add.w  #12,a7
        tst.w  d0
        bne.s  *+$68
        move.w #7,-(a7)
        move.l $00000A20.w,-(a7)
        
;***************************************************************************************
;neue ramanzeige
		org  romorg+$06076+lang_dif
		load ramorg+$06076+lang_dif
		moveq	#10,d7		;7 Stelle statt 5 = total 11 ("kB" und 2xleerschlag)

;nicht "ST RAM" sondern "FASTRAM"  
		org  romorg+$062b2+lang_dif
		load ramorg+$062b2+lang_dif
		dc.b	"FASTRAM"

		org  romorg+$063d4+lang_dif
		load ramorg+$063d4+lang_dif
...
L03CE:		MOVEM.L (A7)+,D3-d4 
      		RTS 
                blk.b	romorg+$06476-*+lang_dif,$ff
;======================================================================================
; .28
...
exc_loop2:	move.l  a3,(a1)+
		...
		move.l  $44e.w,$e8.w		;physbas setzen
       		...
       		dc.w	$f23c,$9000,0,0			;fmove.l #0,fpcr
                bra 	exc_wei
la_rte:         rte
la_rts:         rts                
                blk.b romorg+$004f0-*,$ff
exc_wei:
...
;kein screen change in vertikal blank interrupt
		org  romorg+$00c9a
		load ramorg+$00c9a
		jsr 	romorg+$0761a+lang_dif
		jsr	flopvbl
		bra 	mon_chg

;scsi arbitration
arbitr: 	BSR	  romorg+$01d18
...
;Åberall st high auflîsung 
		...
		moveq #vidmo,d0
		rts
		blk.b romorg+$01172-*,$ff
		rts
		blk.b romorg+$011c8-*,$ff
		rts
		blk.b romorg+$011d0-*,$ff
		rts
		blk.b romorg+$01202-*,$ff

		org  romorg+$01756
		load ramorg+$01756
		move.l #vidmo00,d0
		rts
		blk.b romorg+$01786-*,$ff
		move.l #vidmo00,d0
		rts
		move.l #vidmo00,d0
		rts
		blk.b romorg+$017a6-*,$ff
		moveq #0,d0
		rts
		blk.b romorg+$017c8-*,$ff
		move.w 4(a7),d0
		and.w #$ff,d0
		sub.w #$100,d0
		neg.w d0
		rts
		blk.b romorg+$017f6-*,$ff
		move.w 4(a7),d0
		and.w #$ff,d0
		sub.w #$100,d0
		neg.w d0
		rts
		blk.b romorg+$01824-*,$ff
		moveq #vidmo,d0
		rts
		blk.b romorg+$01848-*,$ff
		moveq #vidmo,d0
		add.b d0,d0
		subx.w d0,d0
		neg.w d0
		rts
		blk.b romorg+$0186c-*,$ff

		org  romorg+$06742+lang_dif
		load ramorg+$06742+lang_dif
		cmp.w #vidmo,d0

		org  romorg+$06972+lang_dif
		load ramorg+$06972+lang_dif
		cmp.w #vidmo,d0

		org  romorg+$06eb2+lang_dif
		load ramorg+$06eb2+lang_dif
		move.l #vidmo00,d5

		org  romorg+$06eba+lang_dif
		load ramorg+$06eba+lang_dif
		and.w #7,d5

		org  romorg+$06eec+lang_dif
		load ramorg+$06eec+lang_dif
		move.l #vidmo00,d4

		org  romorg+$06fae+lang_dif
		load ramorg+$06fae+lang_dif
		move.l #vidmo00,d0

		org  romorg+$07066+lang_dif
		load ramorg+$07066+lang_dif
		move.l #vidmo00,d5

		org  romorg+$070fe+lang_dif
		load ramorg+$070fe+lang_dif
		move.l #vidmo00,d7

		org  romorg+$07172+lang_dif
		load ramorg+$07172+lang_dif
		move.l #vidmo00,d0
...
;keine tt palette
		org  romorg+$06ea4+lang_dif
		load ramorg+$06ea4+lang_dif
		rts

		org  romorg+$07052+lang_dif
		load ramorg+$07052+lang_dif
		rts
...
;************************************
		...
		org  romorg+$05616+lang_dif
		load ramorg+$05616+lang_dif
		jsr  flopini.l
		
		org  romorg+$05968+lang_dif
		load ramorg+$05968+lang_dif
		jsr  floprd.l
		
		org  romorg+$05db4+lang_dif
		load ramorg+$05db4+lang_dif
		jsr  floprd.l
		
;floppy hardware routinen
		org  romorg+$03ba8+lang_dif
		load ramorg+$03ba8+lang_dif
...
xetv_crit:	equ romorg+$00D92	; etv_critic  Critical Event Handler
xwait_xtc:	equ romorg+$0162E	; wait_tctick  MFP Timer C Ticks warten
...
fdc_rdint2:	btst	#4,gpip.w			;daten?
...
fdc_wrint:      bclr	#6,$fffffa89.w                  ;int off
		movem.l	a0/d0-d1,-(sp)                  ;register sichern
                move.l	cbuffer.w,a0
                move.w	#512,d0                         ;1 sector a 512 byts
fdc_wrint2:	btst	#4,gpip.w			;daten?
...
fdc_vrint:      bclr	#6,$fffffa89.w                  ;int off
		movem.l	a0/d0-d2,-(sp)                  ;register sichern
                move.l	cbuffer.w,a0
                move.w	#512,d0                         ;1 sector a 512 byts
fdc_vrint2:	btst	#4,gpip.w			;daten?
...
floprd21:
		...
		bsr	sendcom
		move.l	xhz_200.w,d6
                add.l	#100,d6				;0.5 sec warten
        cmp.b   #dd,hdinfo+1(a1)
        bne.s   floprd3
        add.l	#200,d6				;1.5 sec warten
...
floprd12:	move.b	hdinfo+1(a1),ldcr			;takt setzen
		move.l  xhz_200.w,d6
		add.l   #100,d6
floprd14:
		cmp.l   xhz_200.w,d6
...
fmt1:
		add.w  #$0300,d0
		move.w	D0,hdinfo(A1)			;dsb.hdinfo setzen
		bsr	select0
...
;**************************************************** Beginn der C-Routinen	***
		...
		blk.b romorg+$045d8+lang_dif-*,$ff

;_bhdv_getbpb
; bios1.fil (ED_BIOS) from TOSPATCH
		org  romorg+$0565c+lang_dif
		load ramorg+$0565c+lang_dif
		LINK	A6,#0 
...
B100D:		MOVEM.L	(A7)+,A4-A5/D6-D7 
		UNLK	A6
		RTS 
		blk.b	romorg+$058ae+lang_dif-*,$ff		

;_bios floppy write (_dorwabs)
; mostly bios2.fil (ED_BIOS) from TOSPATCH, except no check for odd address
		org  romorg+$05b1e+lang_dif
		load ramorg+$05b1e+lang_dif
		LINK	A6,#$FFF4 
		MOVEM.L	A5/D2-D7,-(A7)
		MOVE.W	#$77AA,A5 
		TST.W	$10(A6) 
...
B2014:		TST.L	(A7)+ 
		...
		UNLK	A6
		RTS 
		blk.b	romorg+$05d2a+lang_dif-*,$ff

; disable boot sector execution (KILL_BOOT patch from TOSPATCH)
		org  romorg+$05d9c+lang_dif
		load ramorg+$05d9c+lang_dif
		dc.b  $60

; correct MSDOS-Format (MS_DOS from TOSPATCH)
		org  romorg+$2A3C2+lang_dif
		load ramorg+$2A3C2+lang_dif
        add.w      #12,a7
        move.l     #$EB34904E,(a5)
...
;cache clear (un)nîtig bei dma (bei 60er sicher nîtig)
		...
		dc.w cpusha
		bra.s romorg+$01a1a
		blk.b romorg+$01a1a-*,$ff

		org  romorg+$07d4e+lang_dif
		load ramorg+$07d4e+lang_dif
		dc.w cpusha
		nop
		bra.s romorg+$07d5a+lang_dif
		blk.b romorg+$07d5a+lang_dif-*,$ff

		org  romorg+$0d300+lang_dif
		load ramorg+$0d300+lang_dif
		dc.w cpusha
		nop
		bra.s romorg+$0d30c+lang_dif
		blk.b romorg+$0d30c+lang_dif-*,$ff
...
cache_ein:	move.l 	d0,-(sp)
		...
		blk.b romorg+$012fe-*,$ff

;***************************************************
;neuer kaltstart
		org  romorg+$0380c+lang_dif
		load ramorg+$0380c+lang_dif

kaltstart:	move.w	#$2700,sr
		move.l	#$ffc040,d0	;no cache serialized
 		dc.w	movec,dtt1	;transparent translation daten 1
		dc.w	movec,itt1	;transparent translation intstruction 1
 		dc.w	movec,dtt0	;transparent translation daten 0
		dc.w	movec,itt0	;transparent translation intstruction 0
		moveq	#,d0	;instruction cache on
        dc.w    movec,cacr      ;cache off
		dc.w	cinva		;caches invalid
...
loopclr:	clr.l	(a0)+
		...
		clr.l   (a0)+
		dbf	d0,loopclr
		move.l	romorg+$00004,a0	;resetvektor holen
		jmp	(a0)
		
		blk.b romorg+$0387c+lang_dif-*,$ff
...
scsiwrlb:
		...
		bmi.s	scsiwrdone
        move.b  (a1)+,(a0)      ;byt verschieben
		bra scsiwrloop   ;wiederholen bis fertig
scsiwrdone:
...
scsirdlb:
		...
		bmi.s 	scsirddone
		move.b  (a0),(a1)+		;byt verschieben
		bra scsirdloop		;wiederholen bis fertig
scsirddone:
		bset	#0,sctr2.w		;count0 on
		move.b	(a0),(a1)+		;dummy byt nachschieben
		move.l  a1,d2
...
bomb_fa5:	lea	tb2(PC),A0	;zeiger auf text
		...
		bsr	string_out	;(PC-4)=
		move.l	Buserr.w,a4		;buserrorvektor sichern
		...
		bsr	reg_aus
bomb_fa6:	move.l	a4,Buserr.w		;alter buserrorvektor
...
bomb_fa7:	move.l	(A1)+,D2	;wert holen
		...
		cmp.w   #140,d7		;vector number = 140?
		beq.s   exitcont
...
exitcont:
		move.l     proc_usp.w,a0
		move.l     a0,usp
		movem.l    proc_regs.w,d0-d7/a0-a7
		rte
...
m64init:        lea	romorg+$70000,a0           ;source emulator.bin mach64 pci
...
m64copy:	move.l	(a0)+,(a1)+            ;copieren
		...
		move.l  (a0)+,(a1)+
		move.l	(a0)+,(a1)+
		dbf	d0,m64copy             ;wiederholen bis fertig
		move.l  a6,-(a7)		   ;push return address from caller
		jmp	(a2)                   ;einsprung
...
isainit:	lea	romorg+$72000,a0		;source emulator.bin mach64 isa
...
check_hardware:
		...
		moveq      #90,d0
		cmp.l      #$00400000,d4
		bcc.s      check_hardware1
		moveq      #3,d5
		bra        check_hardware11
check_hardware1:
		move.l     #check_hardware2,Buserr.w
		tst.b      $FFFF8000.w
		nop
		bra.s      check_hardware3
check_hardware2:
		add.w      #$30,d5
		bra        check_hardware11
check_hardware3:
		move.l     #check_hardware4,Buserr.w
		move.b     d0,$fffffa15.w
		nop
		tst.b      $fffffa01.w
		nop
		cmp.b      $fffffa15.w,d0
		beq.s      check_hardware5
		sub.w      #$200,d5
check_hardware4:
		add.w      #$300,d5
check_hardware5:
		move.l     #check_hardware6,Buserr.w
		move.b     d0,$fffffa95.w ; IMRB of TTMFP
		nop
		tst.b      $fffffa01.w ; THO: BUG? should be GPIP of TTMFP
		nop
		cmp.b      $fffffa95.w,d0
		beq.s      check_hardware7
		sub.w      #$2000,d5
check_hardware6:
		add.w      #$3000,d5
check_hardware7:
		move.l     #check_hardware8,Buserr.w
		clr.b      psgsel.w
		nop
		move.b     d0,psgwr.w
		nop
		...
		nop
		cmp.b      psgrd.w,d0
		beq.s      check_hardware9
		sub.l      #$00020000,d5
check_hardware8:
		add.l      #$00030000,d5
check_hardware9:
		move.l     #check_hardware10,Buserr.w
		move.b     #13,$ffff8961
		nop
		cmp.b      #$80,$ffff8963
		beq.s      check_hardware11
		sub.l      #$00200000,d5
check_hardware10:
		add.l      #$00300000,d5
check_hardware11:
...
; calculate crc (same logic as in check_rom_crc)
crccheck2:
		move.l     a5,a0
		move.l     d7,d2
		clr.w      d0
		clr.w      d1
		clr.w      d3
		move.l     romorg+$1738,a2 ; fetch address of crc table from check_rom_crc function
ckcrc2:
		move.w     d0,d1
		lsl.w      #8,d0
		lsr.w      #8,d1
...
memconf_error4:
		move.l     #$87654321,d3
		divu       #$1234,d3
		dbf        d2,memconf_error4
		dbf        d0,memconf_error3
		lea        startup_stk.w,a7  ; reset supervisor stack
		and.l      #$0022FFFF,d5
		beq.s      goback
		stop       #$2700
...
pcicheck_1:
		cmp.b      #$FF,0(a1,d1.l)
		beq.s      pcicheck_3
		move.l     #$0100000F,48(a1,d1.l)
		move.b     4(a1,d1.l),d7
		bset       #1,4(a1,d1.l)
		cmp.l      $8F000000,d0
		bne.s      pcicheck_2
		lea        pcicheck_2(pc),a5
		jmp        $8F000004
pcicheck_2:
		move.b     d7,4(a1,d1.l)
		clr.b      48(a1,d1.l)
